...
%
% Main comparison:
%   Static GRiFFIN-style threshold
%   vs
%   SAC receiver-level dynamic threshold
%
% Scenarios:
%   Random 1.1:
...
% Change this if you want 10,15,20,...,70
% malicious_ratios = 0.10:0.05:0.70;
...
outDir = fullfile(projectDir, "griffin_exact_six_scenarios_results");
...
fprintf("\n===== EXACT GRiFFIN SIX-SCENARIO SIMULATION STARTED =====\n");
...
for scenarioIdx = 1:length(scenarioList)
    ...
    for ratioIdx = 1:length(malicious_ratios)
        ...
        if SAVE_DETAIL
            detailLog = zeros(totalDetailRows, 35);
            detailIdx = 0;
        end

        if SAVE_THETA_LOG
            thetaLog = zeros(totalThetaRows, 13);
            thetaIdx = 0;
        end
        ...
        for formationType = formationList
            ...
            %% ===== Malicious UAV IDs =====
            % Same malicious set can act as target/receiver/jury depending on scenario flags.
            ...
            for t = 1:T
                ...
                %% ===== Reported distance matrix for GRIFFIN Phase 2 =====
                    % Phase 2 should use reported inter-UAV distance, not RSSI distance.
                ...
                if cfg.attack_class == "random"
                    ...
                    % Random attack:
                    % malicious target lies about position without planning.
                    for m = malicious_ids
                ...
                elseif cfg.attack_class == "intelligent"
                    ...
                    % Intelligent attack:
                    % malicious target chooses receiver-specific forged position
                ...
                for receiver = 1:params.N
                    ...
                    receiver_is_malicious = ismember(receiver, malicious_ids);
                        ...
                        for target = 1:params.N
                            ...
                            if ~staticP1Fail(target)
                                ...
                                [p2fail, numMalJuries, rejectVotes] = runPhase2MajorityVote( ...
                                    ...
                                    malicious_ids, cfg.jury_lies, juryRejectThreshold);
                ...
                if mod(t,20) == 0
                    fprintf("scenario=%s | ratio=%d%% | env=%d | form=%d | t=%03d | mean theta=%.3f\n", ...
                        scenarioName, malicious_percent, envId, formationType, t, mean(currentTheta));
                end
            end
            ...
            summaryRows = addSummaryRow(summaryRows, cfg, malicious_ratio, malicious_percent, ...
        ...
        if SAVE_DETAIL
            ...
            detailFile = fullfile(outDir, sprintf("%s_%d_detail.csv", scenarioName, malicious_percent));
        ...
        if SAVE_THETA_LOG
            ...
            thetaFile = fullfile(outDir, sprintf("%s_%d_sac_theta_log.csv", scenarioName, malicious_percent));
        ...
        writetable(runningSummary, fullfile(outDir, "griffin_exact_summary_RUNNING.csv"));
    end
end
...
summaryFile = fullfile(outDir, "griffin_exact_six_scenarios_summary.csv");
writetable(summaryTable, summaryFile);

fprintf("\n====================================================\n");
fprintf("DONE - EXACT GRiFFIN SIX-SCENARIO SIMULATION COMPLETE\n");
...
function summaryRows = addSummaryRow(summaryRows, cfg, ratio_setting, malicious_percent, ...
